...
jsPsych.plugins["video-overlay-button-response"] = (function () {
  ...
  plugin.trial = function (display_element, trial) {
    ...
    if (isSafari) {
      ...
      console.log(
        "Turning off preloading for Safari. Additional delay possible."
      );
    }
    if (!video_preload_blob) {
      for (var i = 0; i < trial.stimulus.length; i++) {
        ...
        if (type == "mov") {
          console.warn(
            "Warning: video-overlay-button-response plugin does not reliably support .mov files."
          );
        }
        video_html +=
    ...
    if (trial.overlay != null) {
      overlay_html =
        '<image id="jspsych-video-overlay-button-response-overlay"';
      if (trial.width) {
      ...
      overlay_html +=
        'style = "position: absolute;  margin-left: -' + trial.width + 'px;"';
      overlay_html += "/image>";
    ...
    if (Array.isArray(trial.button_html)) {
      if (trial.button_html.length == trial.choices.length) {
        ...
        console.error(
          "Error in video-overlay-button-response plugin. The length of the button_html array does not equal the length of the choices array"
        );
      }
    } else {
    ...
    for (var i = 0; i < trial.choices.length; i++) {
      ...
      video_html_2 +=
        '<div class="jspsych-video-overlay-button-response-button" style="cursor: pointer; display: inline-block; margin:' +
        trial.margin_vertical +
        " " +
        trial.margin_horizontal +
        '" id="jspsych-video-overlay-button-response-button-' +
        i +
        '" data-choice="' +
        i +
        '">' +
    ...
    var video_element = display_element.querySelector(
      "#jspsych-video-overlay-button-response-stimulus"
    );
    var overlay_element = display_element.querySelector(
      "#jspsych-video-overlay-button-response-overlay"
    );
    ...
    if (trial.overlay != null) {
      hidden = true;
      _.range(0, trial.overlay_time * 1000, trial.blink_time).forEach((t) => {
        if (hidden) {
          jsPsych.pluginAPI.setTimeout(() => {
    ...
    function end_trial() {
      ...
      display_element
        .querySelector("#jspsych-video-overlay-button-response-stimulus")
        .pause();
    ...
    function disable_buttons() {
      var btns = document.querySelectorAll(
        ".jspsych-video-overlay-button-response-button"
      );
