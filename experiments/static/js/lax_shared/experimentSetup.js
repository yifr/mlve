...
function setupExperiment() {
  ...
  function getStimListFromMongo(config, callback) { //called in experiments where stimulus subsets are stored in mongo database
    ...
    socket.emit('getStim',
      {
        gameID: gameID,
        stimColName: config.stimColName, // should check if exists (but could do above when choosing whether or not to call this function)
      });
  ...
  function preloadStimuliForExperiment(callback) {
    ...
    fetchingFunction = config.stimColName ? getStimListFromMongo : getAllStimuliToLoadForExperimentTrials;
    ...
    fetchingFunction(config, (allStimuliIdsForTrials) => {
      ...
      allStimuliURLsForTrials = getStimuliIdsToStimuliURLs(
        config,
        domain,
      ...
      // Preload stimuli if necessary before constructing trials
