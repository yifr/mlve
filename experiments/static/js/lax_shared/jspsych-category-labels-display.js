...
jsPsych.plugins["jspsych-category-labels-display"] = (function () {
  var plugin = {};

  // jsPsych.pluginAPI.registerPreload('tower-display', 'stimulus');

  plugin.info = {
    name: "jspsych-category-labels-display",
    parameters: {
      stimURL: {
  ...
  function initInterface(display_element, trial) {
    ...
    const IMAGE_GRID_SIZE = 100; 

    var html = `
    <div class="container center-block">
      <div class="row">
        <p> <strong> Upcoming images:</strong> </p>
        <div class="demo-instruction"></br>Here you will see the images you are about to describe.</div>
      </div>
      <div class="row">
        <table style="margin: auto; width: 50%; id="jspsych-category-labels-table" 
    ...
    `;
      ...
      for (var col = 0; col < ncols; col++) {
        ...
        if (!(imgId)) {
          html +=
          `<td id="table-grid-img-id-${col}" style="padding: 10px 10px; border: 1px solid #555;">
          ...
          html +=
            `<td id="table-grid-img-id-${imgId}" style="padding: 10px 10px; border: 1px solid #555;">
              <div id="${src}" style="width: ${IMAGE_GRID_SIZE}px; height: ${IMAGE_GRID_SIZE}px;">
                <img src=${src} style="width: ${IMAGE_GRID_SIZE}px; height: ${IMAGE_GRID_SIZE}px"> </img>
              </div>
            </td>
            `
          }
    ...
    html += `
        ...
        <div class="row" id="jspsych-category-labels-container" css="display: none;">
        ...
        <div class="row">
            <div class="col-lg-6" id="image-canvas-container">
              <img src="" class="img-rounded" style="border: solid 1px black; max-width: 500px;" id="image"></img>
              <div class="demo-instruction">Here is the image you are describing.</div>
            </div>

              <div class="col-lg-6"> 
                <div class="row">
                  <div class="demo-instruction">Please write your instructions here.</div>
                  ...
                  <div class="demo-instruction"> Click <strong>add instruction step</strong> (or press the Enter Key) to add a new what and where step.</div>
                  <div style="margin-top:10px;">
                    <input type="submit" id="jspsych-next" class="jspsych-btn jspsych-next" value="${trial.button_label}"></input>
                    <div class="demo-instruction">Only click here when you are done writing instructions for this image.</div>
                  </div>
                  <br><br>
  ...
  var add_row = function(trial, display_element, rows) {
    ...
    if (n_rows < trial.maxRows){
      ...
      newRow.setAttribute("style", "margin: 1em;")
  ...
  plugin.trial = function (display_element, trial) {
    ...
    if (trial.demo) {
      // display_element.querySelector(".demo-instruction").classList.add('hidden');
      $('.demo-instruction').show();
      //$('#jspsych-category-labels-table').hide();
    }
    else {
