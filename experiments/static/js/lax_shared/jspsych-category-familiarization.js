  ...
  function initInterface(display_element, trial) {
    ...
    for (var row = 0; row < nrows; row++) {
      ...
      for (var col = 0; col < ncols; col++) {
        ...
        html +=
          `<td id="table-grid-img-id-${idx}" style="padding: 10px 10px; border: 1px solid #555;">
            <div id="${imgPath}" style="width: ${IMAGE_GRID_SIZE}px; height: ${IMAGE_GRID_SIZE}px;">
              <img src=${imgPath} id="img-${idx}" style="width: ${IMAGE_GRID_SIZE}px; height: ${IMAGE_GRID_SIZE}px; display: ${display}"> </img>
            </div>
          </td>
          `
        idx += 1;
    ...
    var next = function() {      
      ...
      if (imageGridIndex < trial.stimBatch.length) {
        if (imageGridIndex > 0) {
            ...
            currImg.style.backgroundColor = "#10ff00";
          }
        return;
