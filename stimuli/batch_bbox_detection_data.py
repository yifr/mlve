            ...
            for i in range(scenes_per_group * 2):
                try:
                    ...
                    mask_path = os.path.join(
                        scene_dir, "masks", f"Image{frame_idx:04d}.png"
                    )
                    ...
                    probe_location, bounding_box, mask_idx = get_probe_location(
                        masks, probe_touching
                    )
                    obj_shape_data = config_data["objects"][f"h1_{mask_idx}"][
                        "shape_params"
                    ]
                    obj_rotation_data = config_data["objects"][f"h1_{mask_idx}"][
                        "rotation_matrix"
                    ][frame_idx]
                    obj_location_data = config_data["objects"][f"h1_{mask_idx}"][
                        "location"
                    ][frame_idx]
                    obj_texture_data = config_data["objects"][f"h1_{mask_idx}"][
                        "texture"
                    ]
                    ...
                    trial_data = {
                        ...
                        "mask_idx": mask_idx,
                        "obj_shape_data": obj_shape_data,
                        "obj_location_data": obj_location_data,
